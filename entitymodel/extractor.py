...
def build_dataframe():
    ...
    data = []
    paths = ['/home/steffen/Uni/masterthesis/label/products/','/home/steffen/Uni/masterthesis/label/nonproducts/']
    url_path = '/home/steffen/Uni/masterthesis/label/filemaps/'
    label = 1

    '''
    load pattern file
    '''
#    csvfile = open('entitymodel/results/patterns.csv', 'r')
#    pattern_reader = csv.reader(csvfile, delimiter=';')
#    patterns = next(pattern_reader)  # file contains only one line
    ...
    for path in paths:
        ...
        for folder in folders:
            ...
            '''
            get all urls for corresponding id
            '''
            try:
                f_url = open(url_path + folder + '.csv')
                reader = csv.reader(f_url, delimiter=',')
                file_url_dict = {r[0]: r[1] for r in reader}
                f_url.close()
            except:
                continue
            ...
            for filename in os.listdir(path + folder):
                ...
                uuid = filename[0:end]
                url = file_url_dict[uuid]
                # coverCount = patternCover(url,patterns)
                # isCovered = 1 if coverCount > 0 else 0

                '''
                Entity model features
                '''
                f = open(path + folder + '/' + filename, 'rb')  # rb prevents decode error
                try:
                    c = eval(folder)
                    me = c(f.read())
                    name = me.get_name()
                ...
                page_count += 1
                #data.append([folder.lower(),url,isCovered,coverCount,name,rating_count,rating_value,has_image,identifier_count,list_length[0],list_length[1],table_length[0],table_length[1],description_length,label])
                data.append([folder.lower(),url,name,rating_count,rating_value,has_image,identifier_count,list_length[0],list_length[1],table_length[0],table_length[1],description_length,label])
